...
class CategoryModel extends Model
{
    ...
    function handleCategoryTreeSelect($childerArr, $depth = 0)
    {
        $listHTML = "";
        ...
        foreach ($childerArr as $child) {
            ...
            $listHTML .= "<option value='$idDm'>$prefix $tenDm</option>";

            if (!empty($child['children'])) {
                $listHTML .= $this->handleCategoryTreeSelect($child['children'], $depth + 1); 
            }
        }
    ...
    public function handleCategorylevelSelect($status)
    {
        $listCtgr = $this->getAllCategories($status);
        $categoryTree = $this->createCategoryTree($listCtgr, 0);
        return $this->handleCategoryTreeSelect($categoryTree);
    }
